...
<div class="hotel-item col-lg-3 col-md-3 col-sm-4 col-xs-12" data-stars="<?= CHtml::encode($data['star']) ?>" data-name="<?= CHtml::encode(strtolower($data['name'])) ?>" data-price="<?= CHtml::encode($data['price']) ?>">
    <div class="hotel-item-body card-panel">
        <div class="img-container">
            <a class="ajax-link" href="<?php echo Yii::app()->createUrl('reservation/hotels/view/'.urlencode($country)/*.'/'.urlencode($city)*/.'/'.urlencode($data['name']).'/'.urlencode($data['traviaID']).'/'.urlencode($searchID));?>"></a>
            <img src="<?php echo CHtml::encode($data['image']['src']);?>" alt="<?php echo CHtml::encode($data['image']['tag']);?>">
        </div>
        <h6 class="name"><a href="<?php echo Yii::app()->createUrl('reservation/hotels/view/'.urlencode($country)/*.'/'.urlencode($city)*/.'/'.urlencode($data['name']).'/'.urlencode($data['traviaID']).'/'.urlencode($searchID));?>"><?php echo CHtml::encode($data['name']);?></a></h6>
        ...
        <a class="ajax-link" href="<?php echo Yii::app()->createUrl('reservation/hotels/view/'.urlencode($country)/*.'/'.urlencode($city)*/.'/'.urlencode($data['name']).'/'.urlencode($data['traviaID']).'/'.urlencode($searchID));?>" class="pull-left">جزئیات...</a>
    </div>
    <?php
    echo CHtml::hiddenField('',$data['rooms'],array('class'=>'rooms'));
    echo CHtml::hiddenField('',$data['searchID'],array('class'=>'searchID'));
    ?>
</div>
