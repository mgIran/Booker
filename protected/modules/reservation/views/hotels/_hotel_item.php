...
<div class="hotel-item col-lg-3 col-md-3 col-sm-4 col-xs-12" data-stars="<?= CHtml::encode($data['star']) ?>" data-name="<?= CHtml::encode(strtolower($data['name'])) ?>" data-price="<?= CHtml::encode($data['price']) ?>">
    <div class="hotel-item-body card-panel">
        <div class="img-container">
            <a href="<?php echo Yii::app()->createUrl('reservation/hotels/view/'.urlencode($country)/*.'/'.urlencode($city)*/.'/'.urlencode($data['name']).'/'.urlencode($data['traviaID']).'/'.urlencode($searchID));?>"></a>
        ...
        <a href="<?php echo Yii::app()->createUrl('reservation/hotels/view/'.urlencode($country)/*.'/'.urlencode($city)*/.'/'.urlencode($data['name']).'/'.urlencode($data['traviaID']).'/'.urlencode($searchID));?>" class="pull-left">جزئیات...</a>
    </div>
</div>
