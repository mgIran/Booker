...
$originCity = Airports::getFieldByIATA(Yii::app()->session['origin'], 'city');
$destinationCity = Airports::getFieldByIATA(Yii::app()->session['destination'], 'city');
?>
            ...
            <ul class="collapsible" data-collapsible="accordion">
                <li>
                    <div class="collapsible-header search-info">
                        <span><?php echo $originCity;?> به <?php echo $destinationCity;?></span> |
        ...
        <div id="flights-container" class="flights-container hidden">
            ...
            <div id="flights-list-views">
                ...
                <?php $this->widget('zii.widgets.CListView', array(
                    ...
                    'itemView'=>'_non_domestic_flight_item',
                    ...
                    'afterAjaxUpdate'=>'function(id, data){
                        ...
                        slider.noUiSlider.on("change", function(values, handle) {
                            $(".package-item tbody tr").each(function(){
                                ...
                                if(price < minPrice)
                                    $(this).parents(".package-item").addClass("hidden-filter");
                                else if(price > maxPrice)
                                    $(this).parents(".package-item").addClass("hidden-filter");
                                else
                                    $(this).parents(".package-item").removeClass("hidden-filter");
                            });
                        });
...
<?php Yii::app()->clientScript->registerScript('select-flight', "
    $('body').on('click', '.select-flight', function(){
        ...
        $(this).parents('tr').addClass('green lighten-5');
    ...
    $('body').on('click', '#show-all', function(){
        $('#flights-list .package-item').each(function(index, item){
            ...
            $(this).find('tr').removeClass('green lighten-5');
        });
...
<?php Yii::app()->clientScript->registerScript('search-flights',"
    $.fn.yiiListView.update('flights-list');
...
<?php Yii::app()->clientScript->registerScript('show-flight-details',"
    $('body').on('click', '.flight-details-link', function(){
        ...
        $.each(flight.legs, function(key, value){
            departureTime = getTimeFromString(value.departureTime);
            arrivalTime = getTimeFromString(value.arrivalTime);
            ...
            var html =
                '<div class=\"leg-item\">'+
                    ...
                    '<div class=\"total-flight-time\">'+
                        '<small>مدت پرواز: '+getHoursAndMinutes(value.flightTime).hour+' ساعت و '+getHoursAndMinutes(value.flightTime).minute+' دقیقه</small>'+
                ...
                if(flight.legs[key + 1] != undefined){
                    ...
                    html += '<div class=\"total-info blue-text text-darken-2\">مدت کل سفر: '+getHoursAndMinutes(travelTime).hour+' ساعت و '+getHoursAndMinutes(travelTime).minute+' دقیقه</div>';
