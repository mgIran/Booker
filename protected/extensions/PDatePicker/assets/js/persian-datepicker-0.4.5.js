...
( function () {(function ($) {
  ...
  var ClassConfig = {
    ...
    show: function () {
      ...
      this.element.main.addClass('picker--focused picker--opened');
    ...
    hide: function () {
      if (this._viewed) {
        ...
        this.element.main.removeClass('picker--focused picker--opened');
     ...
     */
    destroy: function () {
      this.inputElem.removeClass(this.cssClass);
    ...
    updateRange: function (newRange) {
      if (newRange.maxDate)
        this.maxDate = newRange.maxDate;
      if (newRange.minDate)
        this.minDate = newRange.minDate;

      this.dayPicker._updateView();
    },

    enableOneWayHighlight: function (enabled) {
      this.oneWayHighlight = enabled;
      if (enabled) {
        this.element.main.find('.picker__table td').removeClass('highlight-end highlight-mid');
        this.element.main.addClass('one-way-highlight').removeClass('two-way-highlight');
      } else {
        this.element.main.addClass('two-way-highlight').removeClass('one-way-highlight');
      }
    },

    updatePage: function (yearAndMonth) {
      this.state.view.year = yearAndMonth[0];
      this.state.view.month = yearAndMonth[1];
      this.state.view.unixDate = moment(yearAndMonth[0] + '-' + yearAndMonth[1] + '-1', 'jYYYY-jMM-jDD').valueOf();
      this.dayPicker._updateView();
    },
    /**
     * @memberOf ClassDatepicker.ClassConfig
  ...
  var TEMPLATE = {
    ...
    "<span class='picker__button--close'><i class='icon zq-cancel'></i></span>" + //
    "</div>" +//
    ...
    "<div class='{{css.btnNext}}'><span>ماه بعدی</span></div>" + //
    "<div class='{{css.btnPrev}}'><span>ماه قبلی</span></div>" + //
    "<div class='{{css.btnSwitch}}' ><span class='j-month'>{{btnSwitchText}}</span><span class='g-month'></span></div>" + //
    "</div>",
    ...
    "<thead><tr><th class='picker__weekday'>شنبه</th><th class='picker__weekday'>یک‌شنبه</th><th class='picker__weekday'>دو‌شنبه</th><th class='picker__weekday'>سه‌شنبه</th><th class='picker__weekday'>چهار‌شنبه</th><th class='picker__weekday'>پنج‌شنبه</th><th class='picker__weekday'>جمعه</th></tr></thead>" +
    "<tbody><tr><td /><td/><td/><td/><td/><td/><td/></tr><tr><td/><td/><td/><td/><td/><td/><td/></tr><tr><td/><td/><td/><td/><td/><td/><td/></tr><tr><td/><td/><td/><td/><td/><td/><td/></tr><tr><td/><td/><td/><td/><td/><td/><td/></tr><tr><td/><td/><td/><td/><td/><td/><td/></tr></tbody></table>" + //
    "</div>"
  }
  'use strict';
  /**
...
// cDon't clobber any existing jQuery.browser in case it's different
  ...
  var ClassMonthGrid = {
    ...
    _markToday: function () {
      var self = this;
      var todate;
      var serverToday = this.datepicker.today;

      if (serverToday)
        todate = new persianDate(serverToday);
      else
        todate = new persianDate();
    ...
    selectDate: function (unixDate) {
      ...
      this.highlightFrom(unixDate);
      self.markSelectedDate(unixDate);
      var returnUnixDate = $(this.datepicker.highlightPartner).attr('unixdate');
      if (self.datepicker.returnInput && returnUnixDate && returnUnixDate < this.datepicker.state.selected.unixDate) {
        $(self.datepicker.returnInput).
        persianDatepicker('updatePage',
          [this.datepicker.state.selected.year, this.datepicker.state.selected.month]
        );
      }
    ...
    highlightFrom: function (unixDate) {
      var self = this;
      var partnerValue;

      if (!self.datepicker.initalHighlight &&
        !$(self.datepicker.element.main).prop('touched') &&
        !$(this.datepicker.partnerRoot).prop('touched')) {
        return;
      }
      if (parseInt($(self.datepicker.highlightPartner).attr('unixDate')))
        partnerValue = parseInt($(self.datepicker.highlightPartner).attr('unixDate'));
      else if ($(self.datepicker.highlightPartner).val() && this.datepicker.initalHighlight)
        partnerValue = moment($(self.datepicker.highlightPartner).val(), 'YYYY-MM-DD').valueOf();
      else
        partnerValue = 0;

      var startDate, endDate;
      startDate = firstSecondOfDay(unixDate);
      endDate = firstSecondOfDay(partnerValue);
      if (self.datepicker.highlightType === 'end') {
        startDate = firstSecondOfDay(partnerValue);
        endDate = firstSecondOfDay(unixDate);
      }

      var partnerTarget = self.datepicker.partnerRoot;
      var partnerDayList = $(partnerTarget + ' .picker__day');

      //console.log('from:' + moment(startDate).format('jDD'));
      //console.log('to:' + moment(endDate).format('jDD'));
      highlightDaysList(self.daysList, startDate, endDate, false, self.datepicker.oneWayHighlight);
      highlightDaysList(partnerDayList, startDate, endDate, true, self.datepicker.oneWayHighlight);

      function highlightDaysList(daysList, start, end, isReturn, oneWayHighlight) {
        clearHighlights(daysList);
        if (end < start || oneWayHighlight) {
          $(daysList).filter(function () {
            return $(this).attr('unixdate') == start
          }).parent().addClass('highlight-start');
          return;
        }
        $.each(daysList, function (index, value) {
          var viewItemUnix = parseInt($(value).attr("unixDate"));
          if (viewItemUnix === startDate) {
            $(this).parent().addClass('highlight-start');
            if (endDate === startDate)
              $(this).parent().addClass('highlight-end');
          } else if (viewItemUnix === endDate) {
            $(this).parent().addClass('highlight-end');
          } else if (viewItemUnix > startDate && viewItemUnix < endDate) {
            $(this).parent().addClass('highlight-mid');
          } else {
            $(this).parent().removeClass('highlight-start highlight-end highlight-mid');
          }
        });
      }

      function clearHighlights(daysList) {
        $(daysList[0]).
        closest('.picker__table').
        find('td').
        removeClass('highlight-start highlight-end highlight-mid');
      }

      function firstSecondOfDay(unixDate) {
      ...
      "default": {
        ...
        renderDays: function (self) {
          ...
          var addSpan = function (day, month, year, cssClass) {
            ...
            if (dayPartUnixTime == 1490124600000)
              dayPartUnixTime = 1490128200000; //fixs the bug in firefox date engine.
            var dayWrapper = $("<div/>")
              .addClass('picker__day')
              .addClass(cssClass)
            ...
            moment.locale('en');
            var spanG = $('<span/>')
              .text(moment(dayPartUnixTime).format('D'))
              .addClass('g-day')
              .appendTo(dayWrapper);
            if (moment(dayPartUnixTime).format('D') === '1')
              spanG.addClass('month-start');
            moment.locale('fa');

            self.daysList.push(dayWrapper);
          }
          var t = new persianDate();
          self.daysCount = t.daysInMonth(self.state.year, self.state.month);
          self.firstWeekDayOfMonth = t.getFirstWeekDayOfMonth(self.state.year, self.state.month);
          ...
          self.daysBox.find('tbody tr:last-child').addClass('empty-week');
          if (self.firstWeekDayOfMonth === 6 && self.daysCount >= 31 ||
            self.firstWeekDayOfMonth === 7 && self.daysCount >= 30) {
            self.daysBox.find('tbody tr:last-child').removeClass('empty-week');
          }
          ...
          $(self.daysBox).find("td").each(function (index) {
            ...
            var thisUnix = $(this).find('.picker__day').data("unixDate");


            if (self.datepicker.state._filetredDate) {
              if (self.datepicker.minDate && self.datepicker.maxDate) {
                if (thisUnix >= self.datepicker.minDate && thisUnix <= self.datepicker.maxDate) {
                  $(this).addClass(self.cssClass.disbaled);
                } else {
                  $(this).removeClass(self.cssClass.disbaled);
                }
              } else if (self.datepicker.minDate) {
                if (thisUnix < self.datepicker.minDate) {
                  $(this).addClass(self.cssClass.disbaled);
                } else {
                  $(this).removeClass(self.cssClass.disbaled);
                }
              } else if (self.datepicker.maxDate) {
                if (thisUnix <= self.datepicker.maxDate) {
                  $(this).removeClass(self.cssClass.disbaled);
                }
              }
            } else {
          ...
          $(self.daysBox).find("td").not('.disabled').find('.picker__day').click(function (e) {
          ...
          $(self.daysBox).find('td.disabled').find('.picker__day').click(function () {
            return false;
          });
  ...
  var ViewsDatePicker = {
    ...
    views: {
      ...
      "default": {
         ...
         */
        render: function (self, calledFromInit) {
          var viewData = {
            css: self.cssClass,
            title: self.title,
          ...
          if (self.targetId)
            self.element.main.attr('id', self.targetId);
          ...
          self.container.navigator = $(self.element.main).find('.' + self.cssClass.navigator);
          self.container.dayView = $(self.element.main).find('.' + self.cssClass.dayView);
        ...
        fixPosition: function (self) {
          if (!self._inlineView) {
            ...
            var inputY;
            if (self.alignByRightSide) {
              inputY = ($(window).width() - (self.inputElem.offset().left + self.inputElem.outerWidth()));
              ...
              inputY = self.inputElem.offset().left;
              if (self.position === "auto") {
                var inputHeight = self.fullHeight(self.inputElem);
                self.element.main.css({
                  top: (inputX + inputHeight) + 'px',
  ...
  var ClassDatepicker = {
     ...
     * @returns {ClassDatepicker}
     */
    updateNavigator: function (switchStr, dateObj) {
      if (this.navigator) {
        this.navigator.updateSwitchBtn(this._formatDigit(switchStr), dateObj);
      }
      return this;
    },
    ...
    selectDate: function (unixDate) {
      ...
      if (this.returnInput)
        this.returnInput.persianDatepicker('updateRange', { minDate: unixDate });

      self.onSelect(unixDate, this);
      if (self.autoClose) {
        self.element.main.removeClass('picker--opened');
      }
      if (self.autoOpenPartner && self.returnInput) {
        self.returnInput.pDatepicker('show');
      }
      $(self.element.main).prop('touched', true);
      return this;
    },
    ...
    _attachEvents: function () {
      ...
      self.inputElem.on('focus click' ,function () {
        self.show();
      });
      ...
      $(".picker__button--close").click(function (e) {
        self.inputElem.blur();
        self.hide();
      });
    ...
    _updateInputElement: function () {
      ...
      // Update Alt Field
      self.altField.
      attr('unixDate', self.state.selected.unixDate).
      val(self.altFieldFormatter(self.state.selected.unixDate)).
      trigger('change');
    ...
    _defineOnInitState: function () {
      ...
      if ($(this.$container)[0].nodeName == 'INPUT') {
        var startOfThisMonth = $(this.altField).val() || this.today;
        garegurianDate = new Date(startOfThisMonth).valueOf() || startOfThisMonth*1000;
        this.$container = $('body');
      }
      else {
    ...
    init: function () {
      ...
      this.view.render(this, true);
      this.inputElem.data("datepicker", this);
      this.inputElem.addClass(self.cssClass);
      $(this.altField).eq(0).data('target', '#' + this.targetId);
      this._attachEvents();
      if (this.initalHighlight === true)
        this.dayPicker.mGrid.highlightFrom(this.state.selected.unixDate);
      if (this.highlightType === 'start')
        this.element.main.addClass('start-picker');
      else
        this.element.main.addClass('end-picker');
  ...
  var ClassNavigator = {
     ...
     */
    cssClass: {
      datpickerHeader: "picker__header",
      btnNext: "picker__nav--next",
     ...
     * @returns {ClassNavigator}
     */
    updateSwitchBtn: function (val, dateObj) {
      moment.locale('en');
      var firstOfMonth = moment(dateObj.year + '-' + dateObj.month + '-' + 1, 'jYYYY-jM-jD'),
        endOfMonth = moment(dateObj.year + '-' + dateObj.month + '-' + 28, 'jYYYY-jM-jD'),
        GMonthLabel = (dateObj.month == 10 ? firstOfMonth.format('YYYY') :  endOfMonth.format('YYYY')) + ' ' + endOfMonth.format('MMMM') + ' <span class="month-lable-arrow">-</span> ' + firstOfMonth.format('MMMM');

      moment.locale('fa');

      $(this.element).children('.' + this.cssClass.btnSwitch).find('.j-month').html(val);
      $(this.element).children('.' + this.cssClass.btnSwitch).find('.g-month').html(GMonthLabel);
      return this;
    },
    ...
    _attachEvents: function () {
      ...
      self.element.children("." + self.cssClass.btnPrev).click(function () {
        self._prev();
        self.datepicker.dayPicker.mGrid.highlightFrom(self.datepicker.state.selected.unixDate);
        if (self.datepicker.highlightType === 'start' && !self.datepicker.element.main.prop('touched'))
          $(self.datepicker.partnerRoot).find('.' + self.cssClass.btnPrev).click();
        return false;
      });
      self.element.children("." + self.cssClass.btnNext).click(function () {
        ...
        self.datepicker.dayPicker.mGrid.highlightFrom(self.datepicker.state.selected.unixDate);
        if (self.datepicker.highlightType === 'start' && !self.datepicker.element.main.prop('touched'))
          $(self.datepicker.partnerRoot).find('.' + self.cssClass.btnNext).click();
        return false;
      });
  ...
  var ClassDayPicker = {
    ...
    _updateNavigator: function (year, month) {
      ...
      var titleParts = pdateStr.split(' ');
      pdateStr = titleParts[1] + ' ' + titleParts[0];

      self.datepicker.updateNavigator(pdateStr, {year: year, month: month});
      return this;
    },
     ...
     */
    hide: function () {
      this.container.removeClass('picker--opened');
      return this;
    },
